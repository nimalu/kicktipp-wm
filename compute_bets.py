...
for bet in bets:
    try:
        ...
        print(f"Match not found for bet: {bet}")
        continue
    ...
    print(f"Best prediction: {res}, Expected score: {expected_score:.2f}")

    if bet.home_goals != res[0] or bet.away_goals != res[1]:
        print(
            f"needs to be updated: {bet.home_goals}:{bet.away_goals} -> {res[0]}:{res[1]}"
        )
